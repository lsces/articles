...
<div class="{$outer_div|default:"post"}">
	<div class="floaticon">
		...
		{if $gContent->hasUserPermission( 'p_articles_read_history' ) && !$version && $article.version > 1}
			{smartlink ititle="View History" ipackage=articles ifile="article_history.php" biticon="appointment" article_id=$article.article_id}
		{/if}
		{if $gContent->hasUpdatePermission()}
			<a href="{$smarty.const.ARTICLES_PKG_URL}edit.php?article_id={$article.article_id}">{biticon ipackage="icons" iname="document-properties" ipackage="icons" iexplain=edit}</a>
		{/if}
