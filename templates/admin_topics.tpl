				...
				<tr class="{cycle values="even,odd"}">
					...
					<td style="text-align:center;">
						{if $topics[user].active_topic eq 'n'}
							{smartlink ititle='activate' biticon="list-remove" fActivateTopic=1 topic_id=$topics[user].topic_id}
						{else}
							{smartlink ititle='deactivate' biticon="dialog-ok" fDeactivateTopic=1 topic_id=$topics[user].topic_id}
						{/if}
					</td>
					...
					<td align="right">
						...
						<br />
						<a href="{$smarty.const.ARTICLES_PKG_URL}admin/admin_topics.php?fRemoveTopic=1&amp;topic_id={$topics[user].topic_id}">{biticon ipackage="icons" iname="edit-delete" ipackage="icons" iforce=icon_text iexplain="Remove Topic"}</a>
						<br />
						<a href="{$smarty.const.ARTICLES_PKG_URL}admin/admin_topics.php?fRemoveTopicAll=1&amp;topic_id={$topics[user].topic_id}">{biticon ipackage="icons" iname="edit-delete" ipackage="icons" iforce=icon_text iexplain="Remove Topic and its Articles"}</a>
					</td>
				</tr>
			{sectionelse}
				<tr class="norecords">
